...
- 설명 : 관광지 검색하러가기 버튼을 누르면 지역별 관광지 정보를 검색할 수 있는 페이지로 이동한다. 아래는 추천 장소 3곳을 표시했다.<br>
...
- 설명 : 검색 페이지 로드 시 데이터베이스로부터 지역별 코드를 불러와 검색창의 지역 Select 버튼을 구성하고 이를 통해, 관광지 정보를 지역별 원하는 컨텐츠 별로 조회할 수 있다.<br>
